...
(function (BABYLON) {
    ...
    (function (GLTF2) {
        ...
        */
        ...
        var Spline = (function () {
            function Spline(keyFrames) {
                this._times = new Array(keyFrames.length);
                for (var i = 0; i < keyFrames.length; i++) {
                    this._times[i] = keyFrames[i].time;
                }
                if (keyFrames.length > 1) {
                    this._segments = new Array(keyFrames.length);
                    for (var i = 0; i < keyFrames.length - 1; i++) {
                        var leftKeyFrame = keyFrames[i];
                        var rightKeyFrame = keyFrames[i + 1];
                        var deltaTime = rightKeyFrame.time - leftKeyFrame.time;
                        this._segments[i] = {
                            p0: leftKeyFrame.value,
                            p1: rightKeyFrame.value,
                            m0: leftKeyFrame.outTangent.scale(deltaTime),
                            m1: rightKeyFrame.inTangent.scale(deltaTime),
                        };
                    }
                }
                else {
                    var keyFrame = keyFrames[0];
                    this._segments = [{ p0: keyFrame.value, p1: keyFrame.value, m0: keyFrame.outTangent, m1: keyFrame.inTangent }];
                }
            }
            Spline.prototype.evaluate = function (time) {
                var segmentIndex = 0;
                var segmentTimeOffset = 0;
                if (time > this._times[0]) {
                    for (var i = 1; i < this._times.length; i++) {
                        var upperTimeBound = this._times[i];
                        if (time <= upperTimeBound) {
                            var lowerTimeBound = this._times[i - 1];
                            segmentIndex = i - 1;
                            segmentTimeOffset = (time - lowerTimeBound) / (upperTimeBound - lowerTimeBound);
                            break;
                        }
                    }
                }
                var segment = this._segments[segmentIndex];
                var t = segmentTimeOffset;
                var t2 = t * t;
                var t3 = t2 * t;
                // See https://en.wikipedia.org/wiki/Cubic_Hermite_spline
                return segment.p0.scale(2 * t3 - 3 * t2 + 1).add(segment.m0.scale(t3 - 2 * t2 + t)).add(segment.p1.scale(-2 * t3 + 3 * t2)).add(segment.m1.scale(t3 - t2));
            };
            return Spline;
        }());
        var SplineAnimation = (function () {
            function SplineAnimation(spline, scene) {
                this._spline = spline;
                this._scene = scene;
            }
            SplineAnimation.prototype.beginAnimation = function (target, from, to) {
                var _this = this;
                this._time = from;
                ...
                this._scene.registerBeforeRender(function () {
                    ...
                    _this._updateTarget(target);
                });
            };
            SplineAnimation.prototype._updateTarget = function (target) {
                var value = this._spline.evaluate(this._time);
                target.rotationQuaternion = value;
                target.rotationQuaternion.normalize();
            };
            return SplineAnimation;
        }());
        var createSplineAnimationHack = function (runtime, targetNode, bufferInput, bufferOutput) {
            var splineKeyFrames = new Array(bufferInput.length);
            var bufferOutputOffset = 0;
            for (var j = 0; j < bufferInput.length; j++) {
                var inTangent = BABYLON.Quaternion.FromArray(bufferOutput, bufferOutputOffset);
                bufferOutputOffset += 4;
                var value = BABYLON.Quaternion.FromArray(bufferOutput, bufferOutputOffset);
                bufferOutputOffset += 4;
                var outTangent = BABYLON.Quaternion.FromArray(bufferOutput, bufferOutputOffset);
                bufferOutputOffset += 4;
                splineKeyFrames[j] = { time: bufferInput[j], inTangent: inTangent, value: value, outTangent: outTangent };
            }
            var spline = new Spline(splineKeyFrames);
            var splineAnimation = new SplineAnimation(spline, runtime.babylonScene);
            splineAnimation.beginAnimation(targetNode, 0, bufferInput[bufferInput.length - 1]);
        };
        /**
        ...
        var loadAnimations = function (runtime) {
            ...
            for (var animationIndex = 0; animationIndex < animations.length; animationIndex++) {
                ...
                for (var channelIndex = 0; channelIndex < animation.channels.length; channelIndex++) {
                    ...
                    // TODO: handle non-Float32Array types.
                    var bufferInput = GLTF2.GLTFUtils.GetBufferFromAccessor(runtime, runtime.gltf.accessors[sampler.input]);
                    var bufferOutput = GLTF2.GLTFUtils.GetBufferFromAccessor(runtime, runtime.gltf.accessors[sampler.output]);
                    ...
                    // TEMPORARY HACK - to prove that spline math works
                    if (targetPath === "rotationQuaternion" && sampler.interpolation === "CUBICSPLINE") {
                        createSplineAnimationHack(runtime, targetNode, bufferInput, bufferOutput);
                        continue;
                    }
                    ...
                    for (var j = 0; j < bufferInput.length; j++) {
                        var value = null;
                        if (targetPath === "rotationQuaternion") {
                            value = BABYLON.Quaternion.FromArray(bufferOutput, arrayOffset);
                            arrayOffset += 4;
                        }
                        else {
                            value = BABYLON.Vector3.FromArray(bufferOutput, arrayOffset);
                            arrayOffset += 3;
                        }
                        if (isBone) {
                            var bone = targetNode;
